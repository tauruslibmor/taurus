...
public:
    ...
    void AddTrainingPoint(const double& x, const double& y);
    
    void SetInitHyperParameters(const double& p1, const double& p2, const double& p3);
    
    void Generate();
    
    void WriteTraining(const string& filename);
    
    void LoadTraining(const string& filename);
    
    void Save(const string& filename);
    
    void Evaluate( const double& x, const double& conf_level, double& mean, double& upper_conf, double& lower_conf);
    
    void Evaluate( const std::vector<double>& x, const string filename="GPtesting.m");
    
    void Evaluate( int num_points, const string filename="GPtesting.m");
    
private:
    
    // members
    GaussianProcess* M_gp;
    int M_NumTrainingPoints;
    std::vector<double> M_TrainingPoints_x;
    std::vector<double> M_TrainingPoints_y;
    Eigen::VectorXd M_hyperParams;
    libgp::RProp M_optimizer;
};
