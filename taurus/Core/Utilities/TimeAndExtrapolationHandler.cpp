#include <taurus/Core/Utilities/TimeAndExtrapolationHandler.hpp>
...
void
TimeAndExtrapolationHandler::extrapolate(vector_Type& extrapolation)
{
    switch (M_BDForder) {
        case 1:
            extrapolation = M_states[M_sizeStencil-1]; // u_star = u_n
...
TimeAndExtrapolationHandler::rhsContribution(vector_Type& rhs_bdf)
{
	switch (M_BDForder) {
        case 1:
            rhs_bdf = M_states[M_sizeStencil-1]; // u_rhs = u_n
            break;
        case 2:
            rhs_bdf = (2*M_states[M_sizeStencil-1] - (1.0/2.0)*M_states[M_sizeStencil-2]); // u_rhs = (2*u_n - 0.5*u_{n-1})
            break;
        case 3:
        	rhs_bdf = (3*M_states[M_sizeStencil-1] - (3.0/2.0)*M_states[M_sizeStencil-2] + (1.0/3.0)*M_states[M_sizeStencil-3]); // u_rhs = (3*u_n - 3/2*u_{n-1} + 1/3*u_{n-2})
        	break;
        default:
            break;
    }
