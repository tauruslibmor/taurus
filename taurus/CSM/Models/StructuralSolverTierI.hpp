...
#ifndef STRUCTURALSOLVERTIERI_H_
#define STRUCTURALSOLVERTIERI_H_
...
public:
	...
	// constructor
	StructuralSolverTierI (const GetPot& dataFile, const commPtr_Type& communicator);

	// destructor
    ~StructuralSolverTierI() {};

    void Setup(const meshPtr_Type& mesh, bcPtr_Type bc, bcPtr_Type bcMark, bool export_flag = true);//, exporterPtr_Type& exporter);

    void SetPressureLoad(const double& load);

    void SetExportOffset(const int offsetExportedJac, const int offsetExportedRhsI, const int offsetExportedRhsE, const int offsetExportedSol);

    void GetExportOffset(int& numExportedJac, int& numExportedRhsI, int& numExportedRhsE, int& numExportedSol);
...
private:
    ...
    std::string M_h5_sol_prefix;
    std::string M_h5_resI_prefix;
    std::string M_h5_resE_prefix;

    boost::shared_ptr<EpetraExt::HDF5> M_HDF5_exporter;
    boost::shared_ptr<EpetraExt::HDF5> M_HDF5_exporterSol;
