...
std::chrono::high_resolution_clock::now()-start \
    ...
    std::vector<double> error_sol;
    std::string FEMsnapshots_filename = dataFile("exporter/name_output_file", "SolFEM");
    ...
    for ( int i = 0; i < parameters.size() ; ++i )
    {
        ...
        CSM_tierI.Setup(localMeshPtr, BCh_problem(), BCh_marking(), false );
        CSM_tierI.getMaterialModelPtr()->updateParameters(ParamValues[0], ParamValues[1], ParamValues[2]);
        CSM_tierI.Solve(ParamValues, false, i);
        CSM_tierI.getSolution(solution_TierI);

//        boost::shared_ptr<ExporterHDF5<mesh_Type > > importer;
//        importer.reset ( new  ExporterHDF5<mesh_Type > ( dataFile, FEMsnapshots_filename+EpetraExt::toString(i)) );
//        importer->setMeshProcId ( uFESpace->mesh(), uFESpace->map().commPtr()->MyPID() );
...
//                                                       std::string ("displacementFEM." + iterationString),
//                                                       uFESpace,
//                                                       solution_TierI,
//                                                       UInt (0),
//                                                       LifeV::ExporterData<mesh_Type>::UnsteadyRegime );
//        importer->readVariable ( solutionReader );
        
        
        // Solve Reduced Model
        StructuralSolverTierIII CSM_HROM(dataFile, Comm);
        CSM_HROM.Setup(localMeshPtr, BCh_problem(), BCh_marking(), V, false );
        ...
        *solution_TierIII -= *solution_TierI;
        
        error_sol.push_back(solution_TierIII->norm2() / solution_TierI->norm2());

        mean += (solution_TierIII->norm2() / solution_TierI->norm2());

        if (verbose)
        {
        	std::cout << "\n===============================================================";
        	printf("\n==========  Relative Error configuration %d = %1.3e", i+1, error_sol[i]);
        	std::cout << "\n===============================================================\n";
        }
    }
